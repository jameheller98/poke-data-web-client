...
const ListInfoRegion: React.FC = () => {
  ...
  if (loading) {
    ...
    return (
      <div className="p-3 pt-16 w-full bg-gray-50">
  ...
  return (
    <>
      ...
      <section className="p-3 grid lg:grid-cols-2 gap-x-5">
        <figure>
          <img
            className="rounded-lg shadow-btn m-auto lg:m-0"
        ...
        <div
          className={`grid ${
            version.length === pokedex.length ? 'grid-cols-2' : 'grid-cols-1'
          } gap-3 pt-1 lg:pt-0 lg:grid-cols-1`}
        >
          <TableView
            data={pokedex}
            title="Pokedex"
        ...
        <div className="block pt-3 lg:col-span-2">
...
const TableView: React.FC<{
  ...
  return (
    <article
      className={`grid ${gridCols} gap-y-5 mt-5 text-lg bg-gray-50 bg-opacity-90 pb-5 ${height} shadow-md rounded-t-lg lg:mt-0 ${
        title === 'Location' ? 'lg:grid-cols-4' : null
      }`}
    >
      <h2
        className={`${
          ...
          title === 'Location' ? 'lg:col-end-5' : null
        }`}
      >
        {title}
      </h2>
